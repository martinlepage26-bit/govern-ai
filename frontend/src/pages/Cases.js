  ...
  return (
    <div className="min-h-screen bg-[#f8f9fc] py-12 px-6 md:px-12" data-testid="cases-page">
      <div className="max-w-5xl mx-auto">
        <h1 className="font-serif text-4xl md:text-5xl font-semibold text-[#1a2744] mb-4">{t.cases.title}</h1>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-xs font-medium text-[#6366f1] uppercase tracking-wide">{study.sector}</span>
                    <span className="text-gray-300">·</span>
                    <span className="text-xs text-gray-500 flex items-center gap-1"><Clock className="w-3 h-3" />{study.duration}</span>
                  </div>
                  <h3 className="font-serif text-xl font-semibold text-[#1a2744] mb-2 group-hover:text-[#6366f1] transition-colors">{study.title}</h3>
        ...
        <div className="mt-12 p-6 bg-gradient-to-r from-[#1a2744] to-[#6366f1] rounded-2xl text-white">
          ...
          <Link to="/connect" className="inline-block bg-white text-[#1a2744] px-5 py-2 rounded-lg font-medium hover:bg-gray-100 transition-colors">{t.cases.bookDebrief}</Link>
        </div>
      </div>
        ...
        <>
          ...
          <div className="fixed top-0 right-0 h-screen w-full md:w-[640px] bg-white border-l border-gray-200 shadow-2xl z-[4500] flex flex-col overflow-hidden">
            <div className="flex items-start justify-between gap-4 p-6 border-b border-gray-100 bg-white">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-medium text-[#6366f1] uppercase tracking-wide">{selectedCase.sector}</span>
                  <span className="text-gray-300">·</span>
                  <span className="text-xs text-gray-500">{selectedCase.duration}</span>
                </div>
                <h2 className="font-serif text-2xl font-semibold text-[#1a2744]">{selectedCase.title}</h2>
            ...
            <div className="flex-1 overflow-auto p-6">
              <div className="mb-6">
                <h3 className="font-semibold text-[#1a2744] mb-2">{t.cases.challenge}</h3>
                <p className="text-gray-600">{selectedCase.challenge}</p>
              </div>
              <div className="mb-6">
                <h3 className="font-semibold text-[#1a2744] mb-3">{t.cases.approach}</h3>
                <ul className="space-y-2">
                  {selectedCase.approach.map((item, i) => (
                    <li key={i} className="flex items-start gap-3 text-gray-600 text-sm">
                      <span className="w-5 h-5 rounded-full bg-[#6366f1]/10 flex items-center justify-center flex-shrink-0 mt-0.5"><span className="text-[#6366f1] text-xs font-bold">{i + 1}</span></span>{item}
                    </li>
                  ))}
                </ul>
              </div>
              <div className="mb-6">
                <h3 className="font-semibold text-[#1a2744] mb-3">{t.cases.outcomes}</h3>
                <ul className="space-y-2">
              ...
              <div>
                <h3 className="font-semibold text-[#1a2744] mb-3">{t.cases.deliverables}</h3>
                <div className="flex flex-wrap gap-2">
                  {selectedCase.deliverables.map((item, i) => (
                    <span key={i} className="px-3 py-1.5 bg-[#f8f9fc] border border-gray-200 rounded-full text-sm text-gray-600 flex items-center gap-2">
                      <FileText className="w-3 h-3 text-[#6366f1]" />{item}
                    </span>
                  ))}
                </div>
              </div>
